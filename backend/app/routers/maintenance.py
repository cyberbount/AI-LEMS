...
from app.models import MaintenanceRecord, MaintenanceSchedule, User
...
def complete(item_id: int, db: Db, _: Annotated[User, Depends(require_roles("admin", "manager", "technician"))]):
    ...
    item.status = "completed"; item.completed_at = datetime.utcnow(); db.commit(); db.refresh(item); return item
